...
/**
 * 表示执行尝试的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record RunId(UUID value) implements StrongId {
     ...
     */
    public RunId {
        value = UuidV7.require(value, "RunId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成执行尝试标识。
     *
     * @return 新生成的执行尝试标识
     */
    public static RunId generate() {
        return new RunId(UuidV7.generate());
    }
    ...
    public static RunId parse(String value) {
        return new RunId(UuidV7.parse(value, "RunId"));
    }
}
