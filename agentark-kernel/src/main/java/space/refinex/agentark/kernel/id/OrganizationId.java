...
/**
 * 表示组织的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record OrganizationId(UUID value) implements StrongId {
    ...
    public OrganizationId {
        value = UuidV7.require(value, "OrganizationId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成组织标识。
     *
     * @return 新生成的组织标识
     */
    public static OrganizationId generate() {
        return new OrganizationId(UuidV7.generate());
    }
    ...
    public static OrganizationId parse(String value) {
        return new OrganizationId(UuidV7.parse(value, "OrganizationId"));
    }
}
