...
/**
 * 表示 Skill 的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record SkillId(UUID value) implements StrongId {
     ...
     */
    public SkillId {
        value = UuidV7.require(value, "SkillId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成 Skill 标识。
     *
     * @return 新生成的 Skill 标识
     */
    public static SkillId generate() {
        return new SkillId(UuidV7.generate());
    }
