...
/**
 * 表示项目的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record ProjectId(UUID value) implements StrongId {
     ...
     */
    public ProjectId {
        value = UuidV7.require(value, "ProjectId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成项目标识。
     *
     * @return 新生成的项目标识
     */
    public static ProjectId generate() {
        return new ProjectId(UuidV7.generate());
    }
    ...
    public static ProjectId parse(String value) {
        return new ProjectId(UuidV7.parse(value, "ProjectId"));
    }
}
