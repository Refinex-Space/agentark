...
/**
 * 表示人工审批的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record ApprovalId(UUID value) implements StrongId {
     ...
     */
    public ApprovalId {
        value = UuidV7.require(value, "ApprovalId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成人工审批标识。
     *
     * @return 新生成的人工审批标识
     */
    public static ApprovalId generate() {
        return new ApprovalId(UuidV7.generate());
    }
     ...
     */
    public static ApprovalId parse(String value) {
        return new ApprovalId(UuidV7.parse(value, "ApprovalId"));
    }
}
