...
/**
 * 表示轮次的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record TurnId(UUID value) implements StrongId {
     ...
     */
    public TurnId {
        value = UuidV7.require(value, "TurnId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成轮次标识。
     *
     * @return 新生成的轮次标识
     */
    public static TurnId generate() {
        return new TurnId(UuidV7.generate());
    }
    ...
    public static TurnId parse(String value) {
        return new TurnId(UuidV7.parse(value, "TurnId"));
    }
}
