...
/**
 * 表示调度任务的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record JobId(UUID value) implements StrongId {
     ...
     */
    public JobId {
        value = UuidV7.require(value, "JobId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成调度任务标识。
     *
     * @return 新生成的调度任务标识
     */
    public static JobId generate() {
        return new JobId(UuidV7.generate());
    }
    ...
    public static JobId parse(String value) {
        return new JobId(UuidV7.parse(value, "JobId"));
    }
}
