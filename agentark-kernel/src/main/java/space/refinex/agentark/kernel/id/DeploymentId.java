...
/**
 * 表示部署的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record DeploymentId(UUID value) implements StrongId {
     ...
     */
    public DeploymentId {
        value = UuidV7.require(value, "DeploymentId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成部署标识。
     *
     * @return 新生成的部署标识
     */
    public static DeploymentId generate() {
        return new DeploymentId(UuidV7.generate());
    }
    ...
    public static DeploymentId parse(String value) {
        return new DeploymentId(UuidV7.parse(value, "DeploymentId"));
    }
}
