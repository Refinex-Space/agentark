...
public class MybatisGovernanceRepository implements GovernanceRepository {
    ...
    public List<AuditEvent> listAudit(
        ...
        int limit) {
        return mapper.listAudit(
                organizationId.value(), projectId.value(), before.orElse(null),
                beforeId.map(EventId::value).orElse(null), limit)
    ...
    public void insertPriceTable(PriceTable table, String actor) {
        mapper.insertPriceTable(new PriceTableRow(
            table.id(), table.organizationId().value(), table.projectId().value(), table.key(),
            table.name(), table.status(), table.version(), table.createdAt(), table.updatedAt()),
            actor);
    }
    ...
    public boolean ingestUsage(UsageLedgerEntry entry) {
        ...
        Instant start = entry.occurredAt().atZone(ZoneOffset.UTC).toLocalDate()
            .atStartOfDay(ZoneOffset.UTC).toInstant();
        Instant end = start.plus(Duration.ofDays(1));
        mapper.upsertUsageAggregate(new UsageAggregateDelta(
            EventId.generate().value(), entry.organizationId().value(), entry.projectId().value(),
            start, end, entry.projectId().asString(), entry.provider(),
            entry.model().orElse("none"), entry.inputTokens(), entry.outputTokens(),
            entry.cachedTokens(), entry.embeddingTokens(), entry.toolCalls(),
            entry.sandboxDurationMs(), entry.estimated() ? 1 : 0, entry.costAmount(),
            entry.currency().orElse("XXX"), entry.priceTableVersionId().orElse(null),
            entry.ingestedAt()));
        return true;
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<UsageLedgerEntry> listUsageLedger(
    ...
    public List<UsageAggregate> listUsageAggregates(
        ...
        return mapper.listUsageAggregates(projectId.value(), from, to, limit).stream()
            .map(row -> new UsageAggregate(
                row.periodStart(), row.periodEnd(), row.dimensionType(), row.dimensionRef(),
                row.provider(), row.modelKey(), row.inputTokens(), row.outputTokens(),
                row.cachedTokens(), row.embeddingTokens(), row.toolCalls(),
                row.sandboxDurationMs(), row.estimatedRecords(), row.sourceRecords(),
                row.costAmount(), row.currency()))
            .toList();
    }
    ...
    public QuotaDecision reserveQuota(
        ...
        Duration ttl,
        Instant now) {
        Optional<QuotaPolicyRow> locked = mapper.lockQuotaPolicy(
            organizationId.value(), projectId.value(), scopeType.name(), scopeRef, metric.name(), now);
        if (locked.isEmpty()) {
            return new QuotaDecision(
                Optional.empty(), true, false, Optional.empty(), Optional.empty(),
        ...
        if (existing.isPresent()) {
            ...
            return new QuotaDecision(
                Optional.of(row.id()), active, false,
                Optional.of(BudgetAction.valueOf(policy.budgetAction())),
                Optional.of(policy.version()), Optional.empty(), Optional.of(row.expiresAt()));
        }
        mapper.expireReservations(policy.id(), now);
        BigDecimal used = metric == QuotaMetric.CONCURRENT_RUN
            ? BigDecimal.ZERO
            : mapper.sumWindowUsage(
                projectId.value(), scopeType.name(), scopeRef, metric.name(),
                now.minusSeconds(policy.windowSeconds()), now);
        ...
        if (exceeded && policy.enforcement().equals("HARD")) {
            return new QuotaDecision(
                ...
                Optional.of(BudgetAction.valueOf(policy.budgetAction())),
                Optional.of(policy.version()), Optional.of(remaining.max(BigDecimal.ZERO)),
                Optional.empty());
        }
        UUID reservationId = EventId.generate().value();
        Instant expiresAt = now.plus(ttl);
        mapper.insertReservation(new ReservationInsert(
            reservationId, organizationId.value(), projectId.value(), policy.id(),
            idempotencyKey, subjectRef, amount, expiresAt, now));
        return new QuotaDecision(
            Optional.of(reservationId), true, exceeded,
            Optional.of(BudgetAction.valueOf(policy.budgetAction())),
            Optional.of(policy.version()), Optional.of(remaining.subtract(amount).max(BigDecimal.ZERO)),
    ...
    public void insertDataset(
        ...
        mapper.insertDataset(new DatasetRow(
            dataset.id(), organizationId.value(), projectId.value(), dataset.key(), dataset.name(),
            dataset.description().orElse(null), dataset.status(), dataset.version(), now), actor);
        mapper.insertDatasetVersion(new DatasetVersionRow(
            version.id(), organizationId.value(), projectId.value(), dataset.id(),
            version.versionNumber(), json(version.schema()), bytes(version.contentHash()), now), actor);
        for (EvaluationTestCase item : cases) {
            mapper.insertTestCase(new TestCaseRow(
                item.id(), organizationId.value(), projectId.value(), version.id(), item.key(),
                item.inputObjectUri(), bytes(item.inputContentHash()), json(item.expected()),
                bytes(item.expectedContentHash()), item.weight(), now), actor);
        }
    }
    ...
    public List<EvaluationDataset> listDatasets(ProjectId projectId, int limit) {
        return mapper.listDatasets(projectId.value(), limit).stream()
            .map(row -> new EvaluationDataset(
                row.id(), row.datasetKey(), row.name(), Optional.ofNullable(row.description()),
    ...
    public void insertEvaluator(
        ...
        mapper.insertEvaluator(new EvaluatorRow(
            evaluator.id(), organizationId.value(), projectId.value(), evaluator.key(),
            evaluator.name(), evaluator.status(), evaluator.version(), now), actor);
        mapper.insertEvaluatorVersion(new EvaluatorVersionRow(
            version.id(), organizationId.value(), projectId.value(), evaluator.id(),
            version.versionNumber(), version.type().name(), json(version.config()),
            bytes(version.contentHash()), now), actor);
    }
    ...
    public List<Evaluator> listEvaluators(ProjectId projectId, int limit) {
        return mapper.listEvaluators(projectId.value(), limit).stream()
            .map(row -> new Evaluator(
                row.id(), row.evaluatorKey(), row.name(), row.status(), row.version()))
            .toList();
    }
    ...
    @Override
    @Transactional
    public void insertEvaluationRun(
        EvaluationRun run,
        ...
        mapper.insertEvaluationRun(new EvaluationRunRow(
            run.id(), organizationId.value(), projectId.value(),
            run.candidateRevisionId().value(), run.candidateSnapshotId().value(),
            run.datasetVersionId(), run.evaluatorVersionId(), run.provider(),
            run.model().orElse(null), run.threshold(), run.baselineRunId().orElse(null),
            run.status().name(), run.totalScore().orElse(null),
            run.regressionDelta().orElse(null), run.createdAt(), run.completedAt().orElse(null)), actor);
        for (EvaluationScore score : scores) {
            mapper.insertEvaluationScore(new EvaluationScoreRow(
                score.id(), organizationId.value(), projectId.value(), score.runId(),
                score.testCaseId(), score.metricKey(), score.score(), score.passed(),
                json(score.details()), run.completedAt().orElse(run.createdAt())));
        }
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public ReleaseGateDecision evaluateReleaseGate(
        OrganizationId organizationId,
        ProjectId projectId,
        AgentId agentId,
        EnvironmentId environmentId,
        RevisionId revisionId) {
        Optional<ReleaseGateRow> match = mapper.findActiveReleaseGate(
            organizationId.value(), projectId.value(), agentId.value(), environmentId.value());
        if (match.isEmpty()) {
            return new ReleaseGateDecision(
                true, false, Optional.empty(), Optional.empty(), "no.active.gate");
        }
        ReleaseGateRow gate = match.orElseThrow();
        Optional<UUID> passing = mapper.findPassingEvaluation(
            projectId.value(), revisionId.value(), gate.datasetVersionId(),
            gate.evaluatorVersionId(), gate.threshold());
        if (passing.isPresent()) {
            return new ReleaseGateDecision(
                true, false, Optional.of(gate.id()), passing, "evaluation.passed");
        }
        boolean soft = gate.enforcement().equals("SOFT");
        return new ReleaseGateDecision(
            soft, soft, Optional.of(gate.id()), Optional.empty(), "evaluation.required");
    }
    ...
    private AuditRow auditRow(AuditEvent event) {
        return new AuditRow(
            event.id().value(), event.sourceEventId(), event.sourcePlane().name(),
            event.organizationId().map(OrganizationId::value).orElse(null),
            event.projectId().map(ProjectId::value).orElse(null), event.principalType().name(),
            event.principalRef(), event.scopeType().name(), event.scopeRef(), event.action(),
            event.result().name(), event.resourceType(), event.resourceRef(),
            event.diffSummary().isEmpty() ? null : json(event.diffSummary()),
            event.policyVersion().orElse(null), event.roleVersion().orElse(null),
            event.traceId().orElse(null), event.requestId().orElse(null),
    ...
    private AuditEvent audit(AuditRow row) {
        return new AuditEvent(
            new EventId(row.id()), row.sourceEventId(), AuditPlane.valueOf(row.sourcePlane()),
            Optional.ofNullable(row.organizationId()).map(OrganizationId::new),
            Optional.ofNullable(row.projectId()).map(ProjectId::new),
            AuditPrincipalType.valueOf(row.principalType()), row.principalRef(),
            AuditScopeType.valueOf(row.scopeType()), row.scopeRef(), row.action(),
            AuditResult.valueOf(row.result()), row.resourceType(), row.resourceRef(),
            row.diffSummaryJson() == null ? Map.of() : map(row.diffSummaryJson()),
            Optional.ofNullable(row.policyVersion()), Optional.ofNullable(row.roleVersion()),
            Optional.ofNullable(row.traceId()), Optional.ofNullable(row.requestId()),
    ...
    private PriceTable priceTable(PriceTableRow row) {
        return new PriceTable(
            ...
            row.priceKey(), row.name(), row.status(), row.version(), row.createdAt(), row.updatedAt());
    }
    ...
    private PriceVersionRow priceVersionRow(PriceTableVersion value) {
        return new PriceVersionRow(
            ...
            value.priceTableId(), value.versionNumber(), value.currency(), value.effectiveFrom(),
            json(value.entries()), bytes(value.contentHash()), value.createdAt());
    }
    ...
    private PriceTableVersion priceVersion(PriceVersionRow row) {
        return new PriceTableVersion(
            row.id(), new OrganizationId(row.organizationId()), new ProjectId(row.projectId()),
            row.priceTableId(), row.versionNumber(), row.currency(), row.effectiveFrom(),
            decimalMap(row.entriesJson()), checksum(row.contentHash()), row.createdAt());
    }

    /** 将 Usage 领域模型转换为数据库行。 */
    private UsageRow usageRow(UsageLedgerEntry value) {
        return new UsageRow(
            value.id(), value.sourcePlane(), value.sourceRecordId(),
            value.organizationId().value(), value.projectId().value(),
            ...
            value.runId().map(RunId::value).orElse(null), value.usageType().name(),
            value.provider(), value.model().orElse(null), value.tool().orElse(null),
            value.inputTokens(), value.outputTokens(), value.cachedTokens(),
            value.embeddingTokens(), value.toolCalls(), value.sandboxDurationMs(),
            value.estimated(), value.priceTableVersionId().orElse(null),
            value.currency().orElse(null), value.costAmount(), value.occurredAt(),
            value.ingestedAt());
    }
    ...
    private UsageLedgerEntry usage(UsageRow row) {
        return new UsageLedgerEntry(
            row.id(), row.sourcePlane(), row.sourceRecordId(),
            ...
            Optional.ofNullable(row.runId()).map(RunId::new), UsageType.valueOf(row.usageType()),
            row.provider(), Optional.ofNullable(row.model()), Optional.ofNullable(row.tool()),
            row.inputTokens(), row.outputTokens(), row.cachedTokens(), row.embeddingTokens(),
            row.toolCalls(), row.sandboxDurationMs(), row.estimated(),
            Optional.ofNullable(row.priceTableVersionId()), Optional.ofNullable(row.currency()),
            row.costAmount(), row.occurredAt(), row.ingestedAt());
    }
    ...
    private QuotaPolicyRow quotaPolicyRow(QuotaPolicy value, Instant createdAt) {
        return new QuotaPolicyRow(
            value.id(), value.organizationId().value(), value.projectId().value(),
            value.scopeType().name(), value.scopeRef(), value.metric().name(),
            value.enforcement().name(), value.limitValue(), value.windowSeconds().orElse(null),
            value.budgetAction().name(), value.effectiveFrom(), value.effectiveUntil().orElse(null),
            value.status(), value.version(), createdAt);
    }
    ...
    private QuotaPolicy quotaPolicy(QuotaPolicyRow row) {
        return new QuotaPolicy(
            row.id(), new OrganizationId(row.organizationId()), new ProjectId(row.projectId()),
            QuotaScopeType.valueOf(row.scopeType()), row.scopeRef(),
            QuotaMetric.valueOf(row.metric()), QuotaEnforcement.valueOf(row.enforcement()),
            row.limitValue(), Optional.ofNullable(row.windowSeconds()),
            BudgetAction.valueOf(row.budgetAction()), row.effectiveFrom(),
            Optional.ofNullable(row.effectiveUntil()), row.status(), row.version());
    }

    /** 将数据库 Dataset Version 行转换为领域模型。 */
    private DatasetVersion datasetVersion(DatasetVersionRow row) {
        return new DatasetVersion(
            row.id(), row.datasetId(), row.versionNumber(), map(row.schemaJson()),
    ...
    private EvaluationTestCase testCase(TestCaseRow row) {
        return new EvaluationTestCase(
            row.id(), row.datasetVersionId(), row.caseKey(), row.inputObjectUri(),
            checksum(row.inputContentHash()), map(row.expectedJson()),
            checksum(row.expectedContentHash()), row.weight());
    }
    ...
    private EvaluatorVersion evaluatorVersion(EvaluatorVersionRow row) {
        return new EvaluatorVersion(
            row.id(), row.evaluatorId(), row.versionNumber(),
            EvaluatorType.valueOf(row.evaluatorType()), map(row.configJson()),
            checksum(row.contentHash()));
    }
    ...
    private EvaluationRun evaluationRun(EvaluationRunRow row) {
        return new EvaluationRun(
            row.id(), new RevisionId(row.candidateRevisionId()),
            new SnapshotId(row.candidateSnapshotId()), row.datasetVersionId(),
            row.evaluatorVersionId(), row.provider(), Optional.ofNullable(row.model()),
            row.threshold(), Optional.ofNullable(row.baselineRunId()),
            EvaluationStatus.valueOf(row.status()), Optional.ofNullable(row.totalScore()),
            Optional.ofNullable(row.regressionDelta()), row.createdAt(),
    ...
    private ReleaseGateRow releaseGateRow(ReleaseGate value) {
        return new ReleaseGateRow(
            value.id(), value.organizationId().value(), value.projectId().value(),
            value.agentId().value(), value.environmentId().map(EnvironmentId::value).orElse(null),
            value.datasetVersionId(), value.evaluatorVersionId(), value.threshold(),
            value.enforcement().name(), value.status(), value.version());
    }
    ...
    private ReleaseGate releaseGate(ReleaseGateRow row) {
        return new ReleaseGate(
            row.id(), new OrganizationId(row.organizationId()), new ProjectId(row.projectId()),
            new AgentId(row.agentId()), Optional.ofNullable(row.environmentId()).map(EnvironmentId::new),
            row.datasetVersionId(), row.evaluatorVersionId(), row.threshold(),
            QuotaEnforcement.valueOf(row.enforcement()), row.status(), row.version());
    }
