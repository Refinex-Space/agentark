...
/**
 * 表示环境的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record EnvironmentId(UUID value) implements StrongId {
    ...
    public EnvironmentId {
        value = UuidV7.require(value, "EnvironmentId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成环境标识。
     *
     * @return 新生成的环境标识
     */
    public static EnvironmentId generate() {
        return new EnvironmentId(UuidV7.generate());
    }
     ...
     */
    public static EnvironmentId parse(String value) {
        return new EnvironmentId(UuidV7.parse(value, "EnvironmentId"));
    }
}
