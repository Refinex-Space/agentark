...
AgentArk 部署 Gateway、Control、Runtime、Scheduler 和 Web 五个无状态工作负载。默认 Built-in Identity 模式运行 Identity、Control、Runtime、Scheduler 四个一次性 Flyway Owner Job；切换外部 OIDC 时不运行 Identity Job。MySQL、Redis、Object Store、Qdrant、可选 OIDC、Vault/Secret Manager 和 OTel 后端由外部平台提供；禁止让任一服务使用其他 Owner 的数据库账号。
...
3. 验证 MySQL 备份点、四个最小权限账号、Redis TLS、Object/Qdrant 备份、Built-in Identity SecretRef 或外部 OIDC，以及 Vault 可用性。
4. 先运行 Flyway Job，逐个确认 Identity（若启用）、Control、Runtime、Scheduler Schema History。
