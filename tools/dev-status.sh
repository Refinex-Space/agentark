...
# 同时展示 Core/RAG 与已准备的 Identity 容器，未启动的可选服务不会被创建。
...
if [ "${identity_prepared}" = "true" ]; then
    check_health identity-session http://127.0.0.1:8080/api/v1/auth/session
    check_health identity-jwks http://127.0.0.1:8080/api/v1/auth/jwks
fi
