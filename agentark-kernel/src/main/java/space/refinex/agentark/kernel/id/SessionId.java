...
/**
 * 表示会话的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record SessionId(UUID value) implements StrongId {
     ...
     */
    public SessionId {
        value = UuidV7.require(value, "SessionId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成会话标识。
     *
     * @return 新生成的会话标识
     */
    ...
    public static SessionId parse(String value) {
        return new SessionId(UuidV7.parse(value, "SessionId"));
    }
}
