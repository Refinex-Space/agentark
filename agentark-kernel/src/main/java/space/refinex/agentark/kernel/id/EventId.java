...
/**
 * 表示运行事件的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record EventId(UUID value) implements StrongId {
     ...
     */
    public EventId {
        value = UuidV7.require(value, "EventId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成运行事件标识。
     *
     * @return 新生成的运行事件标识
     */
    public static EventId generate() {
        return new EventId(UuidV7.generate());
    }
    ...
    public static EventId parse(String value) {
        return new EventId(UuidV7.parse(value, "EventId"));
    }
}
