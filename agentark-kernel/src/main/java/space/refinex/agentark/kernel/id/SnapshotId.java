...
/**
 * 表示发布快照的强类型 UUIDv7 标识，防止不同领域标识在编译期被误用。
 *
 * @param value RFC 9562 UUIDv7 原始值
 * @author refinex
 */
public record SnapshotId(UUID value) implements StrongId {
     ...
     */
    public SnapshotId {
        value = UuidV7.require(value, "SnapshotId");
    }

    /**
     * 使用当前 UTC 毫秒时间和安全随机数生成发布快照标识。
     *
     * @return 新生成的发布快照标识
     */
    public static SnapshotId generate() {
        return new SnapshotId(UuidV7.generate());
    }
    ...
    public static SnapshotId parse(String value) {
        return new SnapshotId(UuidV7.parse(value, "SnapshotId"));
    }
}
